...
def store_name(store_id,**kwargs):
    #返回案场名称（用作文件命名）
    store_list = kwargs.get('list')
    if store_list:
        store_name = {}
        for i in range(len(store_list)):
            ...
            store_name[store_dict['store_id']] = store_dict['store_name']
        print(store_name)
        if store_id in store_name:
            print(store_name['cdb978497a'])
            return store_name[store_id]
    else:
        return store_id

#渠道页面
async def qudao_page(store_id):
    #gettoken = await page.evaluate("window.localStorage.getItem('Sense-Token')", force_expr=True)
    #getcompanyid = await page.evaluate("window.localStorage.getItem('Sense-CompanyId')", force_expr=True)
    url = 'https://icloud.sensetime.com/senserealty/recognition/api/sensego/console/v1.0/trade'
    #querystring = {"type":"0","store_id":"ee80854bf0"}
    ...
    res = requests.session()
    store_name_req = res.get(url=url, headers=headers, params=querystring)
    store_name_res = store_name_req.json()
    store_list = store_name_res.get('list')
    store_info = await store_find()
    filename = store_name(store_id,**store_info)
    print("渠道风控写入文件名称：",filename)
    wb = openpyxl.Workbook()
    ws = wb.create_sheet(index=0, title="渠道风控")
    ws.append(["客户姓名","人员id","首次到访抓拍时间","刷证时间"])
    wt = wb.create_sheet(index=1, title="渠道风控详情数据")
    if store_list:
        for i in range(len(store_list)):
            print(store_list[i])
            store_dict = store_list[i]
            ...
            if store_dict['first_capture_time'] and store_dict['verify_time']:
                ws.cell(i+1, i+1).value = store_dict['name']
                ws.append([store_dict['name'],store_dict['id_number'],store_dict['first_capture_time'],store_dict['verify_time']])
                #渠道风控详细记录写入
                ...
                querystring = {"id_number":store_dict['id_number'], "store_id":store_id}
                qudaodetail = res.get(url=url, headers=headers, params=querystring)
                qudaodetailres = qudaodetail.json()
                qudaodetail_list = qudaodetailres.get('list')
                print("渠道风控详细记录", qudaodetail_list)
                wt.append([store_dict['name'],"person_id", "首次到访时间", "到访时间地址", "轨迹数据"])
                if qudaodetail_list:
                    for j in range(len(qudaodetail_list)):
                        print(qudaodetail_list[j])
                        qudao_detail_dict = qudaodetail_list[j]
                        print("渠道风控详细记录2333：",qudao_detail_dict)
                        #案场渠道风控识别记录数据
                        print(qudao_detail_dict['person_id'],qudao_detail_dict['arrived_at'],qudao_detail_dict['arrived_image_url'],
                              qudao_detail_dict['trace'])
                        wt.append([store_dict['name'],qudao_detail_dict['person_id'],qudao_detail_dict['arrived_at'],qudao_detail_dict['arrived_image_url'],str(qudao_detail_dict['trace'])])
                        if j > 5:break
                else:
                    wt.append([store_dict['name'], "person_id", "首次到访时间", "到访时间地址", "轨迹数据"])
                    wt.append([store_dict['name'], "---", "---", "---", "---"])
            else:
                ws.append([store_dict['name'], store_dict['id_number'],'--','--'])
            if i > 50:break
    else:
        ws.append([filename,store_id,"没有渠道风控数据"])
    ws.append(["执行结束"])
